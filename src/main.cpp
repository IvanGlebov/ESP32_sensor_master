...
// Использованные пины сейчас
// digital -> virtual | relay name/relay usage

// 1  -> V10 | Помпа капельного полива
// 14 -> V11 | Верхняя аэропоника (общая на два блока)
// 15 -> V13 | Нижняя аэропоника  (общая на два блока)
// 6  -> V15 | Основной свет блок 1
// 7  -> V16 | Основной свет блок 2
// 8  -> V17 | Красный свет блок 1
// 9  -> V18 | Красный свет блок 2
// 4  -> V19 | Вентилятор блока 1
// 5  -> V20 | Вентилятор блока 2
// 12 -> V21 | Парогенератор блока 1
// 13 -> V22 | Парогенератор блок 2
// 2  -> V23 | Подогрев блока 1
// 3  -> V24 | Подогрев блока 2
// 16 -> V25 | Дренажная помпа (общая на два блока)
// Заняты 1,2,3,4,5,6,7,8,9,_,_,12,13,14,15
// Свободны 10,11
...
void workObj::lightControl()
{
  if (getMode() == automatic)
  {
    ...
    // z1_x, z5_x - all off
    // z2_x, z4_x - red light on
    // z3_x - only main light on
...
void workObj::redLightControl()
{
  if (getMode() == automatic)
  {
    ...
    // logging.println("Z1 :" + String(z1_1));
    // logging.println("Z2 :" + String(z2_1));
    // logging.println("Z3 :" + String(z3_1));
    // logging.println("Z4 :" + String(z4_1));
